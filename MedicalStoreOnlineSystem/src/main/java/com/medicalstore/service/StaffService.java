...
public class StaffService {
	...
	public ResponseEntity<ResponseStructure<Staff>> saveStaff(StaffDTO staff, long adminId, long medicalStoreId) {
		...
		if (medicalStore.getAdmin().getAdminId() == adminId) {
			...
			for (Staff eachStaff : listStaff) {
				if (eachStaff.getMedicalStore().getStoreId() == medicalStoreId)
					throw new AlreadyAddStaffException(
							"This staff already added another medicalStore choose another one...");
			}
			...
			return new ResponseEntity<>(new ResponseStructure<>(HttpStatus.CREATED.value(), "Staff Created...",
					staffDao.saveStaff(staffEntity)), HttpStatus.CREATED);
		}
	...
	public ResponseEntity<ResponseStructure<Staff>> updateStaff(StaffDTO staff, long staffId, long adminId) {
		...
		if (oldStaff.getAdmin().getAdminId() == adminId) {
			...
			return new ResponseEntity<>(
					new ResponseStructure<>(HttpStatus.OK.value(), "Staff updated...", staffDao.saveStaff(staffEntity)),
					HttpStatus.OK);
		}
	...
	public ResponseEntity<ResponseStructure<Staff>> findStaffById(long staffId) {
		...
		return new ResponseEntity<>(
				new ResponseStructure<>(HttpStatus.FOUND.value(), "Staff Found...", staffDao.findStaffById(staffId)),
				HttpStatus.FOUND);
	}

	public ResponseEntity<ResponseStructure<List<Staff>>> findStaffByName(String staffName) {

		return new ResponseEntity<>(new ResponseStructure<>(HttpStatus.FOUND.value(), "Staff Found...",
				staffDao.findStaffByName(staffName)), HttpStatus.FOUND);
	}
	...
	public ResponseEntity<ResponseStructure<Staff>> deleteStaffById(long staffId, long adminId) {
		...
		if (staff.getAdmin().getAdminId() == adminId) {
			...
			return new ResponseEntity<>(new ResponseStructure<>(HttpStatus.GONE.value(), "Staff Found...",
					staffDao.deleteStaffById(staffId)), HttpStatus.GONE);
